...
namespace OmegaWarhead.UI
{
    ...
    public sealed class NukePanel
    {
        ...
        /// <summary>Small author tag appended to panel borders.</summary>
        private const string AuthorTag = "<color=#888888><size=11>By DNT_OF</size></color>";
        ...
        public void RenderIdle(Player player)
        {
            ...
            set.Header.Text = Border(ColorBlue, Localization.PanelTitle);
            set.Info.Text = $"<color={ColorBlue}>{Localization.PanelIdleStatus}</color>\n{Localization.PanelIdleHolder(player.Nickname)}";
            set.Status.Text = $"<color={ColorBlue}>{Localization.PanelIdleStandby}</color>";
            set.Footer.Text = FooterBorder(ColorBlue, Localization.PanelIdleFooter);
        }
        ...
        public void RenderConfirming(Player player, float remainingWindow, float totalWindow)
        {
            ...
            set.Header.Text = Border(ColorYellow, Localization.PanelConfirmHeader);
            set.Info.Text = Localization.PanelConfirmInfo(remainingWindow);
            set.Status.Text = $"<color={ColorYellow}>{BuildBar(remainingWindow, totalWindow, 6)}</color>";
            set.Footer.Text = FooterBorder(ColorYellow, Localization.PanelConfirmFooter);
        }
        ...
        public void RenderLocked(Player player)
        {
            ...
            set.Header.Text = Border(ColorRed, Localization.PanelLockedHeader);
            set.Info.Text = $"<color={ColorRed}>{Localization.PanelLockedInfo}</color>";
            set.Status.Text = $"<color={ColorRed}>{Localization.PanelLockedStatus}</color>";
            set.Footer.Text = FooterBorder(ColorRed, Localization.PanelLockedFooter);
        }
        ...
        private IEnumerator<float> CountingRefreshCoroutine(Player player, NukeSession session)
        {
            while (_panels.ContainsKey(player) && session.State == NukeState.Counting)
            {
                if (_panels.TryGetValue(player, out PanelSet set))
                {
                    ...
                    set.Header.Text = Border(color, Localization.PanelTitle);
                    set.Info.Text = Localization.PanelCountingInfo(session.PointOfNoReturn);
                    set.Status.Text =
                        $"{Localization.PanelCountingStatus(session.RemainingTime)}\n" +
                        $"<color={color}>{BuildBar(session.RemainingTime, session.TotalTime, 12)} {percent}%</color>";
                    set.Footer.Text = FooterBorder(color, Localization.PanelCountingFooter);
                }
        ...
        {
            return $"<color={color}>-----[ {Localization.PanelTipPrefix} ]-----</color>\n<color=#ffffff>{tip}</color>\n{AuthorTag}";
        }
