            ...
            while (target != null && target.IsConnected && target.IsAlive)
            {
                ...
                if (isCounting)
                {
                    ...
                    string timeText = remaining >= 0f
                        ? $"<color=#ffff00><b>{remaining:F0}</b></color> seconds until detonation"
                        : "Reading countdown...";

                    string color = (state.Session?.PointOfNoReturn ?? false) ? "#ff0000" : "#ff4444";

                    text = $"<color=#ff0000><b>[OMEGA Warhead Tracking]</b></color> " +
                           $"Operator <color={color}>{target.Nickname}</color> is at {location}, {timeText} " +
                           $"<color=#ffaa00>Intercept immediately to abort the launch!</color>";
                }
                else
                {
                    text = $"<color=#ffaa00>[Radiation Tracking]</color> Detected {target.Nickname} " +
                           $"carrying radioactive elements. Current location: <color=#ff4444>{location}</color>";
                }
