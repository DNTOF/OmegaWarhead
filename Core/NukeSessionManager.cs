...
namespace OmegaWarhead.Core
{
    ...
    public sealed class NukeSessionManager
    {
        ...
        public static NukeSessionManager Create(Config config)
        {
            if (_instance != null)
            {
            ...
            _instance = new NukeSessionManager(config);
            return _instance;
        }
        ...
        private NukeSessionManager(Config config)
        {
        ...
        private void Subscribe()
        {
            ...
            _launchKeybind = new KeybindSetting(
                ...
                hintDescription: "Press this key to confirm launch while holding the Warhead Launch Controller in your inventory.",
        ...
        public void HandleConfirmAction(Player player)
        {
            if (ActiveSession == null)
            {
                ...
                ActiveSession = new NukeSession(player)
                {
                    ...
                    RemainingTime = _config.CountdownTotalSeconds
                };
                ...
                NukePanel.Instance?.ShowPanel(player);
                NukePanel.Instance?.RenderConfirming(player, _config.ConfirmWindowSeconds, _config.ConfirmWindowSeconds);
                ...
                Log.Info($"[NukeSession] {player.Nickname} first confirmation, entering Confirming state. " +
                         $"Window: {_config.ConfirmWindowSeconds}s.");
                NukeEvents.RaiseConfirmingStarted(ActiveSession);
                return;
            }
        ...
        {
            float remaining = _config.ConfirmWindowSeconds;
            ...
            while (remaining > 0f)
            {
                ...
                NukePanel.Instance?.RenderConfirming(session.Operator, remaining, _config.ConfirmWindowSeconds);
            }
            ...
            NukeEvents.RaiseConfirmingTimedOut(session.Operator);
            ClearSession();
        }
        ...
        private void TransitionToLocked(NukeSession session)
        {
            ...
            // Role reset: detach from original team, set to Tutorial (spectator team,
            // unaffected by original team's win conditions).
            ...
            NukeEvents.RaiseLocked(session);
        ...
        private void TransitionToCounting(NukeSession session)
        {
            ...
            session.RemainingTime = _config.CountdownTotalSeconds;
            session.TotalTime = _config.CountdownTotalSeconds;
            ...
            TrackingBroadcaster.Instance?.StartCountingBroadcast(op, session, _config.CountingTrackIntervalSeconds);
            ...
            // Server-wide CASSIE alert
            Exiled.API.Features.Cassie.Message("WARNING . OMEGA WARHEAD LAUNCH SEQUENCE INITIATED . ALL PERSONNEL EVACUATE", false, false);

            NukeEvents.RaiseCountingStarted(session);

            Log.Info($"[NukeSession] {op.Nickname} entering Counting, countdown {_config.CountdownTotalSeconds}s.");
        }
        ...
        private IEnumerator<float> CountdownCoroutine(NukeSession session)
        {
            while (session.State == NukeState.Counting && session.RemainingTime > 0f)
            {
                ...
                if (!session.PointOfNoReturn &&
                    session.RemainingTime <= _config.PointOfNoReturnSeconds)
                {
                    ...
                    Exiled.API.Features.Cassie.Message("OMEGA WARHEAD POINT OF NO RETURN", false, false);
                    NukeEvents.RaisePointOfNoReturnReached(session);
                }

                // Countdown milestone announcements
                if (Math.Abs(session.RemainingTime - 60f) < 0.01f)
                    Exiled.API.Features.Cassie.Message("60 SECONDS", false, false);
                else if (Math.Abs(session.RemainingTime - 30f) < 0.01f)
                    Exiled.API.Features.Cassie.Message("30 SECONDS", false, false);
                else if (Math.Abs(session.RemainingTime - 10f) < 0.01f)
                    Exiled.API.Features.Cassie.Message("10 .", false, false);
            }
        ...
        private void TransitionToDetonation(NukeSession session)
        {
            ...
            NukeEvents.RaiseDetonating(session);

            // Delayed global kill (ensures all players die; vanilla nuke doesn't cover every zone)
            Timing.RunCoroutine(GlobalKillCoroutine(session, _config.DetonationKillDelaySeconds));
        }
        ...
        private IEnumerator<float> GlobalKillCoroutine(NukeSession session, float delay)
        {
            ...
            NukeEvents.RaiseDetonationCompleted(session);
            ClearSession();
        ...
        private void OnDying(DyingEventArgs ev)
        {
            ...
            NukeEvents.RaiseLaunchAborted(ev.Player);
            ...
            // CASSIE abort announcement
            Exiled.API.Features.Cassie.Message("OMEGA WARHEAD LAUNCH ABORTED", false, false);
        }
